...
namespace UserService.Controllers
{
    ...
    public class OutboxController : ControllerBase
    {
        ...
        public OutboxController(UserDbContext db, ILogger<OutboxController> logger)
        {
            _db = db;
            _logger = logger;
        }
        ...
        {
            // limit max to protect server and prevent accidental large responses
            ...
            {
                _logger.LogInformation("GetUnsent cancelled by request/host. TraceId={TraceId}", HttpContext?.TraceIdentifier);
        ...
        public async Task<IActionResult> MarkSent(Guid id, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                // Use a short linked token so a client disconnect doesn't abort the DB persist immediately.
